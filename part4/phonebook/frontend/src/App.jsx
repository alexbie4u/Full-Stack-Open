...
const App = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (existingPerson) {
      ...
      noteService
        ...
        .catch(error => {
          setErrorMessage(`The length of the name needs to be at least 3 letters, and the numbers needs to be 8 digits.`);
          setTimeout(() => {
            setErrorMessage(null);
          }, 5000);
        });
  ...
  const filteredPersons = useMemo(() => {
    return persons.filter((person) =>
      person && person.name && person.name.toLowerCase().includes(String(filterText).toLowerCase())
    );
  }, [persons, filterText]);
